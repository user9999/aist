...
// User settings
$to = $ADMIN_EMAIL;//"play-fine@yandex.ru";//"user9999@rambler.ru";$ADMIN_EMAIL
$subject = "One click order from  ".$MAINPATH;
...
if (empty($action) && $_GET['title']) {
	...
	<div class='contact-content'>
		...
		<form action='#' style='display:none'>
			...
			<input type='text' id='contact-phone' class='contact-input' name='phone' tabindex='1004' />
			<input type=hidden name=product value='".$_GET['title']."'><br/>";
    ...
    /*
    $output .= "
    <label for='contact-message'>*Адрес доставки:</label>
    <textarea id='contact-message' class='contact-input' name='message' cols='40' rows='3' tabindex='1006'></textarea>
    <br/>";
    */
    if ($extra["form_cc"]) {
        $output .= "
    ...
    $output .= "
			<label>&nbsp;</label>
			<button type='submit' class='contact-send contact-button' tabindex='1006'>Отправить заявку</button>
    ...
    $message.="Заказано: ".$product."\n\n";
    // make sure the token matches
    if ($token === smcf_token($to)) {
        smcf_send($name, $email, $subject, $message, $cc, $phone, $password);
        echo "Спасибо, ваше заказ отправлен. С Вами свяжутся в ближайшее время.";
    }
    else {
...
function smcf_send($name, $email, $subject, $message, $cc,$phone,$password)
{
    ...
    if(!smcf_validate_phone($phone)) {
        $subject .= " - invalid phone";
        $message .= "\n\nBad phone: $phone";
        $mess.="Неверный номер телефона.<br>";
    }
    /*
