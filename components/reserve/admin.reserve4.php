...
$res=mysql_query("select a.*,b.oem,b.name,b.description,b.price,b.special,b.country,c.name as brandname,d.name as modelname from ".$PREFIX."reserved as a,".$PREFIX."catalog_items as b,".$PREFIX."catalog_brands as c,".$PREFIX."catalog_models as d where b.brand_id=c.id and b.model_id=d.id and a.gruz_id=b.name and a.add_date=1 order by user_id");
