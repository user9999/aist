...
if($_POST['db']) {
    $res = mysql_query("SELECT name,subsection_id,description,keywords,section FROM catalog_items");
    while($row = mysql_fetch_row($res)){
        ...
        $imalt=explode(",",$row[3]);
        //var_dump($imalt);
        $res3 = mysql_query("INSERT INTO catalog_items2 SET name='".$row[2]."',linked_item='".$row[0]."',keywords='".$row[3]."',subsection_id=".$row[1].",section=".$row[4].",alt='alt=\"".$imalt[0]."\"'");  
        //echo "INSERT INTO catalog_items2 SET name='".$row[2]."',linked_item='".$row[0]."',keywords='".$row[3]."',subsection_id=".$row[1].",section=".$row[4].",alt='alt=\"".$imalt[0]."\"'<br>";
        }
        */
    }
}
