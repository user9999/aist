...
if($_POST['massfoto']) {
    if(glob("uploaded/source/*.*")) {
        foreach(glob("uploaded/source/*.*") as $curfoto){
            ...
            */
            ...
            $res=mysql_query("select id from catalog_items2 where linked_item='".$curfoto."' limit 1");
            $row = mysql_fetch_row($res);
            if($row[0]) {
                $id =$row[0];
            } else {
                $res2=mysql_query("select model_id,description,keywords,section from catalog_items where name='".$curfoto."' limit 1");
                $row2 = mysql_fetch_row($res2);
                $alt=explode(",", $row2[2]);
                mysql_query("insert into catalog_items2 set name='".$row2[1]."',linked_item='".$curfoto."',keywords='".$row2[2]."',model_id=".$row2[0].",section=".$row2[3].",alt='alt=\"".$alt[0]."\"'");
                $id = mysql_insert_id();
            }
            if ($type && $id) {
                //delete everything
    ...
    if(glob("uploaded/source2/*.*")) {
        foreach(glob("uploaded/source2/*.*") as $curfoto){
            ...
            $res=mysql_query("select id from catalog_items2 where linked_item='".$curfotoid."' limit 1");
...
<?php
if($WATERMARK) {
    $w="<label for=\"small\">Водяной знак</label><input id=small type=checkbox name=small value=1 checked>
