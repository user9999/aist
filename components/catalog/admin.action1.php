...
//edit static page
if (isset($_GET['edit'])) {
    $res = mysql_query("SELECT * FROM catalog_brands WHERE id='{$_GET['edit']}'");
...
if (isset($_POST['frm_name'])) {
  $frm_name=$_POST['frm_name'];
    ...
    if (strlen($frm_name) >= 3) {
        if (!$_POST['editid']) {
            mysql_query("INSERT INTO catalog_brands SET showmenu='$frm_showmenu',name='$frm_name',altname='$frm_altname',position='$frm_position'");
        } else {
            mysql_query("UPDATE catalog_brands SET showmenu='$frm_showmenu',name='$frm_name',altname='$frm_altname',position='$frm_position' WHERE id={$_POST['editid']}");
        }
        header("Location: ?component=catalog");
...
<tr>
<td><b>Альт. название:</b></td><td><input class="textbox" name="frm_altname" type="text" style="width: 100%;" value="<?= $tbl_altname ?>"></td>
</tr>
<tr>
