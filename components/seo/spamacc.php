...
while($spams=mysql_fetch_row($add->query_result)){
	$target['<!--spams-->'].="<tr><td>".$spams[3]."</td><td>".$spams[2]."</td><td>".substr(stripslashes($spams[4]),0,40)."...</td><td><input type=\"submit\" name=\"red[".$spams[0]."]\" value=\"редактировать\" class=\"little\" /><br /><input type=\"submit\" name=\"send[".$spams[0]."]\" value=\"разослать\"  class=\"little\" /></td></tr>";
}
if($_POST['addspam']){
