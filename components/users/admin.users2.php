...
if (isset($_GET['del'])) {
    mysql_query("DELETE FROM ".$PREFIX."menu_users WHERE id='{$_GET['del']}'");
    header("Location: ?component=users&action=2"); 
}

if (isset($_GET['edit'])) {
    $res=mysql_query("SELECT * FROM ".$PREFIX."menu_users WHERE id='{$_GET['edit']}'");
...
if (isset($_POST['frm_name']) && isset($_POST['frm_path'])) {
    ...
    if (!$_POST['editid']) {
        ...
        mysql_query("INSERT INTO ".$PREFIX."menu_users SET text='$frm_name', path='$frm_path', ordering=$frm_order");
        //echo  "INSERT INTO menu_users SET text='$frm_name', path='$frm_path', ordering=$frm_order";
    } else {
        mysql_query("UPDATE ".$PREFIX."menu_users SET text='$frm_name', path='$frm_path', ordering=$frm_order WHERE id={$_POST['editid']}");
        // echo "UPDATE menu_users SET text='$frm_name', path='$frm_path', ordering=$frm_order WHERE id={$_POST['editid']}";
    }
...
<?php
$res=mysql_query("SELECT * FROM ".$PREFIX."menu_users ORDER BY ordering, id");
$num=0;
while ($row=mysql_fetch_row($res)) {
    $num++;
    echo $row[1]." <a href='?component=users&action=2&edit={$row[0]}'>[редактировать]</a> <a href='?component=users&action=2&del={$row[0]}'>[удалить]</a><br />";
}
