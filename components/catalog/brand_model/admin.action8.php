...
if(isset($_FILES['frm_csv']) && $_FILES['frm_csv']['error']==0) {
    ...
    foreach($file as $key=>$value){
        ...
        if($i>=2) {
            mysql_query('update '.$PREFIX.'catalog_items set available=spb+'.$data[1].',msk='.$data[1].' where name="'.$data[0].'"');
        }
        $i++;
    }
    $upctime=time();
...
?>
<br><br>
<h1>Экспорт количества на складе в Москве</h1>
...
<br>
<a href="?component=price&action=5&cmd=partner">обновить информацию для партнеров</a>