...
if (isset($_GET['del'])) {
    mysql_query("DELETE FROM ".$PREFIX."catalog_sections WHERE id='{$_GET['del']}'");
    header("Location: ?component=catalog"); 
}
$tbl_showmenu = 1;
//edit static page
if (isset($_GET['edit'])) {
    $res = mysql_query("SELECT * FROM ".$PREFIX."catalog_sections WHERE id='{$_GET['edit']}'");
...
if (isset($_POST['frm_name'])) {
    ...
    if($frm_parent==0) {
        ...
        $res=mysql_query("select level from ".$PREFIX."catalog_sections where id=".$frm_parent." limit 1");
    ...
    if (strlen($frm_name) >= 3) {
        ...
        if (!$_POST['editid']) {
            //var_dump();
            //die("!!!!!!!!!!!edit");
            mysql_query("INSERT INTO ".$PREFIX."catalog_sections SET name='$frm_name',altname='$frm_altname',level=$level,parent=$frm_parent, keywords='$frm_keywords',description='$frm_keywords',info='$frm_info',showmenu='$frm_showmenu',position='$frm_position'".$qimage);
        } else {
            mysql_query("UPDATE ".$PREFIX."catalog_sections SET name='$frm_name',altname='$frm_altname',level=$level,parent=$frm_parent, keywords='$frm_keywords',description='$frm_keywords',info='$frm_info',showmenu='$frm_showmenu',position='$frm_position'".$qimage." WHERE id={$_POST['editid']}");
...
$sections=array();
///1
$res = mysql_query("SELECT MAX(level) FROM ".$PREFIX."catalog_sections where showmenu=1");
$row=mysql_fetch_row($res);
if($row[0]!==null) {//mysql_num_rows($res)>0
    ...
    $t=substr($t, 0, -1);
    $query="SELECT ".$t." FROM ".$PREFIX."catalog_sections as t1".$join." where t1.parent=0 and t1.showmenu=1";
