...
if (isset($_POST['frm_name']) && isset($_POST['frm_item_id'])) {
    ...
    if (strlen($frm_name) >= 2 && $frm_linked_item) {
        if (!$_POST['editid']) {
            mysql_query("INSERT INTO catalog_items2 SET name = '$frm_name', description = '$frm_description', linked_item = '$frm_linked_item', keywords = '$frm_keywords', alt = '$frm_tags'");
            $id = mysql_insert_id();
        } else {
            mysql_query("UPDATE catalog_items2 SET name = '$frm_name', description = '$frm_description', linked_item = '$frm_linked_item', keywords = '$frm_keywords', alt = '$frm_tags' WHERE id={$_POST['editid']}");
            $id = (int) $_POST['editid'];
        }
        ...
        <tr>
            ...
            <td>
                <select name="frm_item_id">
					...
					FROM catalog_items AS a, catalog_brands AS b, catalog_models AS c
