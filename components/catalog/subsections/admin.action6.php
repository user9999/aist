...
if($_POST['change']) {
    foreach($_POST['name'] as $id=>$val){
        ...
        mysql_query("update catalog_brands set name='".$val."',position=".$_POST['position'][$id]." where id=".$id);
    }

}
if($_GET['brands']) {
    $flag=0;
    $res = mysql_query("SELECT id,name,position FROM catalog_brands where subsection_id={$_GET['brands']} order by position");
...
<?php
$res = mysql_query("SELECT * FROM catalog_sections");
$num = 0;
while ($row = mysql_fetch_row($res)) {
    $num++;
