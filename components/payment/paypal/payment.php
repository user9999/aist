...
//fclose($fp1);
  $res=mysql_query("select pp_verified from ".$PREFIX."payment where id=".$id);
...
while (!feof($fp)) {                     // While not EOF
    ...
    if (strcmp($res, "VERIFIED") == 0) {  // Response contains VERIFIED - process notification
        ...
        mysql_query("update ".$PREFIX."payment set pp_payer_id='{$pp_payer_id}',pp_payer_status='{$pp_payer_status}',pp_payer_email='{$pp_payer_email}',pp_txn_id='{$pp_txn_id}',pp_payment_date='{$pp_payment_date}',pp_payment_status='{$pp_payment_status}',pp_pending_reason='{$pp_pending_reason}',pp_verified='VERIFIED',pay_date='".time()."',payment_gross='{$payment_gross}',mc_gross='{$pp_price}' where id=".$id);
        $res=mysql_query("select a.userid,a.package_id,a.price,b.period from ".$PREFIX."payment as a, ".$PREFIX."packages as b where a.package_id=b.id and a.id=".$id." limit 1");
        ...
        if($pp_price!=$price) {
            mysql_query("update ".$PREFIX."payment set `note`='not_equal' where id=".$id);
        }
        $payed_till=till($period);
        mysql_query("update ".$PREFIX."users set package=$package_id,payed_till='{$payed_till}' where id=".$userid);
