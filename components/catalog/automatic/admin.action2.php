...
if (isset($_GET['edit'])) {
	...
	$res11 = mysql_query("SELECT * FROM ".$PREFIX."catalog_brands WHERE ".$PREFIX."subsection_id='{$_GET['edit']}'");
...
if (isset($_POST['frm_name'])) {
    ...
    if (strlen($frm_name) >= 2) {
		...
		//die("INSERT INTO catalog_subsections SET  name='$frm_name', altname='$frm_altname', img = '$frm_image',sortby='$sort'");
            mysql_query("INSERT INTO ".$PREFIX."catalog_sections SET  name='$frm_name', altname='$frm_altname', img = '$frm_image',sortby='$sort'");
        } else {
		//die("UPDATE catalog_sections SET  name='$frm_name',  altname='$frm_altname', img = '$frm_image',sortby='$sort' WHERE id={$_POST['editid']}");
            mysql_query("UPDATE ".$PREFIX."catalog_sections SET  name='$frm_name',  altname='$frm_altname', img = '$frm_image',sortby='$sort' WHERE id={$_POST['editid']}");
        }
        header("Location: ?component=catalog&action=2");
...
if($_POST['update_pos']){
 //mydump($_POST['pos'],'pkey');
  foreach($_POST['pos'] as $pval=>$pkey){
    ...
    mysql_query("UPDATE ".$PREFIX."catalog_subsections SET position=".$pkey." where id=".$pval);
	if($pkey!=0){
		...
		<tr>
            <td width="130">Раздел:</td><td>
				<select name="frm_brand_id">
					<?php
					$res = mysql_query("SELECT * FROM ".$PREFIX."catalog_sections ORDER BY name");
...
<?php
$res = mysql_query("SELECT * FROM ".$PREFIX."catalog_sections order by id");
$num = 0;
while ($row = mysql_fetch_array($res)) {
	$res1=mysql_query("SELECT id FROM ".$PREFIX."catalog_sections where parent={$row[0]}");
	...
	//<br />
	$res2 = mysql_query("SELECT * FROM ".$PREFIX."catalog_subsections WHERE section_id = $row[0] ORDER by position");
