...
set_css($css);
    $script="<script type='text/javascript' src='".$PATH."/templates/admin.blank/js/image-upload.js'></script>\r\n";
    set_script($script);
$this_script = "?component=news";
...
} else { $short_text = array();//mysql_escape_string
}
...
if($cmd=="edit") {
    if(isset($_REQUEST['news_id'])) {
        ...
        if(isset($_REQUEST['news_cmd_del']) and $_REQUEST['news_cmd_del'] == "Delete" ) {
            $res .= "<p><a href=\"$this_script&cmd=start\">Вернуться к списку новостей</a></p>
					<form action=\"$this_script&cmd=del\" method=\"post\">
		         	<p><b>Вы уверены, что хотите удалить эту новость!!!</b></p>
		         	<table width=\"100%\" border=\"0\" cellspacing=\"0\" cellpadding=\"4\">
					<tr><td>Дата</td><td>Заголовок</td><td>Краткая информация</td><td>Ключевые слова</td></tr>";
            $sql = "SELECT * FROM `".$PREFIX."news` WHERE `id`='".$news_id."'";
            $query = mysql_query($sql)or die("Ошибка при запросе: 1" . mysql_error());
            $news_res = mysql_fetch_array($query);
            $res1=mysql_query("select `title`,`short` from ".$PREFIX."lang_text where rel_id={$news_res['id']} and table_name='news'");
        ...
        if(isset($_REQUEST['news_cmd_edit']) and $_REQUEST['news_cmd_edit'] == "Edit" ) {                
                $sql = "SELECT * FROM `".$PREFIX."news` WHERE `id`='".$news_id."'";
                $query = mysql_query($sql)or die("Ошибка при запросе: 2" . mysql_error());
                $news_res = mysql_fetch_array($query);
                
            $date = $news_res['date'];
            $query = mysql_query("select * from ".$PREFIX."lang_text where rel_id={$news_res['id']}");
            while($res=mysql_fetch_assoc($query)){
                  $title[$res['language']] = $res['title'];
                  $short_text[$res['language']] = $res['short'];
                  $full_text[$res['language']] = stripslashes($res['content']);
                  $keywords[$res['language']]= $res['keywords'];
                  $description[$res['language']]= $res['description'];
            }
...
if($cmd=="del") { //Удаление новости из базы данных
    if(isset($_REQUEST['news_id'])) {
        if($_REQUEST['del'] == "yes") {
             $Query = "DELETE FROM `".$PREFIX."news` WHERE id='".intval($_REQUEST['news_id'])."'";
            $dbResult = mysql_query($Query) or die("Ошибка при запросе: 3" . mysql_error());
            mysql_query("DELETE FROM `".$PREFIX."lang_text` WHERE rel_id='".intval($_REQUEST['news_id'])."' and `table_name`='news'");
            $res .= "новость успешно удалена!<br>";
        }
            $cmd="start";
    }
}
    //Стартовая страница
if($cmd=="start") {
      $res .= "<p><a href=\"$this_script&cmd=add_news\">Добавить новость</a></p>";
       //Определение временных рамок новостей
    $sql = "SELECT `date` FROM `".$PREFIX."news` ORDER BY `date` ASC LIMIT 0,1";
    if($query = mysql_query($sql)) {    $fetch = mysql_fetch_array($query);
    }
    $min_date = $fetch['date'];

    $sql = "SELECT `date` FROM `".$PREFIX."news` ORDER BY `date` DESC LIMIT 0,1";
    if($query = mysql_query($sql)) { $fetch = mysql_fetch_array($query);
    }
    $max_date = $fetch['date'];
    ...
    $query = mysql_query($sql_final);// or die("Ошибка при запросе: 4" . mysql_error());
    ...
    if($max==0) {
        ...
        if($max > $col_mes_on_page) {
            ...
            if($start_page >= $num_page_on_str) {
                ...
                $str .= "<a href=\"news/start_date=".$start_date."&end_date=".$end_date."&page=".$last_page."\">&lt;&lt;</a>&nbsp;";
            }
            for($i = $start_page; $i <= $end_page; $i++){
                if($i == $page) {
                        $str .= "<b>";
                }
                $str .= "<a href=\"news/start_date=".$start_date."&end_date=".$end_date."&page=".$i."\">".$i."</a>&nbsp;";
                if($i==$page) {
                    $str .= "</b>";
                }
            }
            if($end_page < (int)$end) {
                ...
                $str .= "<a href=\"news/start_date=".$start_date."&end_date=".$end_date."&page=".$next_page."\">&gt;&gt;</a>&nbsp;";
            }
              $str .= "<br><br>";
        }
        else{
        ...
        $query = mysql_query($page_sql) or die("Ошибка при запросе: 5" . mysql_error());
        $res .= $str;
        $res .= "<br /><br /><form action=\"$this_script&cmd=edit\" method=\"post\">
				<input type=\"submit\" class='button' name=\"news_cmd_edit\" value=\"Edit\">&nbsp;&nbsp;
				<input type=\"submit\" class='button-reset' name=\"news_cmd_del\" value=\"Delete\"><br>
				<table width=\"100%\" border=\"0\" cellspacing=\"0\" cellpadding=\"4\">
...
if($cmd=="add") {
    if($title and $short_text and $full_text and $keywords ) {
        ...
        if(isset($_REQUEST['upd_id'])) { //Обновление существующей новости
            ...
            mysql_query("UPDATE `".$PREFIX."news` SET `date`='$date' WHERE `id`='$upd_id'");
                
            foreach($title as $lang=>$name){
                //echo "update lang_text SET table_name='news',   title='".$title[$lang]."' , `short`='".mysql_escape_string($short_text[$lang])."', `content`='".mysql_escape_string($full_text[$lang])."',`description`='".htmlspecialchars($description[$lang])."',`keywords`='".htmlspecialchars($keywords[$lang])."'  WHERE `rel_id`=$upd_id and table_name='news'<br><br>";
                mysql_query("update ".$PREFIX."lang_text SET table_name='news',  title='".$title[$lang]."' , `short`='".mysql_escape_string($short_text[$lang])."', `content`='".mysql_escape_string($full_text[$lang])."',`description`='".htmlspecialchars($description[$lang])."',`keywords`='".htmlspecialchars($keywords[$lang])."',pub_date='".time()."'  WHERE `rel_id`=$upd_id and table_name='news' and language='$lang'");
            }                    
            ...
            //$dbtitle=($_POST['send']=='mail')?"<span>".$title."</span>":$title;
            mysql_query("INSERT INTO `".$PREFIX."news` (`date`) VALUES ('$date')");
            $static_id=mysql_insert_id();
            foreach($title as $lang=>$name){
                    //echo "INSERT INTO lang_text SET table_name='news', rel_id=$static_id, language='$lang', title='".$title[$lang]."' , `short`='".mysql_escape_string($short_text[$lang])."', `content`='".mysql_escape_string($full_text[$lang])."',`description`='".htmlspecialchars($description[$lang])."',`keywords`='".htmlspecialchars($keywords[$lang])."'";
                    mysql_query("INSERT INTO ".$PREFIX."lang_text SET table_name='news', rel_id=$static_id, language='$lang', title='".$title[$lang]."' , `short`='".mysql_escape_string($short_text[$lang])."', `content`='".mysql_escape_string($full_text[$lang])."',`description`='".htmlspecialchars($description[$lang])."',`keywords`='".htmlspecialchars($keywords[$lang])."',pub_date='".time()."'");
            }        
...
if($cmd=="send") {
    ...
    $res = "<h1>Новости</h1><p><a href=\"$this_script&cmd=start\">Перейти к списку новостей</a></p>
...
if($cmd=="add_news") {        
    ...
    if(!isset($edit_news) or $edit_news == "") {
           $res .= "<input type=\"submit\" class='button' value=\"Добавить в базу данных\" />&nbsp;<input class='button-reset' type=\"reset\" />";
    } else {
        $res .= "<input type=\"hidden\" name=\"upd_id\" value=\"".$edit_news."\">
            	<input type=\"submit\" class='button' value=\"Внести изменения в базу данных\">";
    }
       $res .= "</form><p><a href=\"$this_script&cmd=start\">Перейти к списку новостей</a></p>";
}
   echo $res;
?>
