...
//delete static page
if (isset($_GET['del'])) {
    mysql_query("DELETE FROM ".$PREFIX."catalog_models WHERE id='{$_GET['del']}'");
...
//edit static page
if (isset($_GET['edit'])) {
    $res = mysql_query("SELECT * FROM ".$PREFIX."catalog_models WHERE id='{$_GET['edit']}'");
...
if (isset($_POST['frm_name'])) {
    ...
    if (strlen($frm_name) >= 2) {
        if (!$_POST['editid']) {
            mysql_query("INSERT INTO ".$PREFIX."catalog_models SET showimg='$frm_showimg',name='$frm_name',altname='$frm_altname',brand_id='$frm_brand_id',image='$frm_image',rusname='{$_POST['frm_rusname']}'");
        } else {
            mysql_query("UPDATE ".$PREFIX."catalog_models SET showimg='$frm_showimg',name='$frm_name',brand_id='$frm_brand_id',altname='$frm_altname',image = '$frm_image',rusname='{$_POST['frm_rusname']}' WHERE id={$_POST['editid']}");
        }
        header("Location: ?component=catalog&action=2");
...
if($_POST['update_pos']){
  foreach($_POST['pos'] as $pval=>$pkey){
    mysql_query("UPDATE ".$PREFIX."catalog_models SET position=".$pkey." where id=".$pval);
  }
}
?>
<br><br>
...
<tr>
<td width=130><b>Альт. название:</b></td><td><input class=textbox name=frm_altname style="width: 100%;" value="<?= $tbl_altname ?>"></td>
</tr>
<tr>
<td width=130><b>Русское название:</b></td><td><input class=textbox name=frm_rusname style="width: 100%;" value="<?= $tbl_rusname ?>"></td>
</tr>
<tr>
<td width=130>Изображение (ссылка):</td><td><input class=textbox name=frm_image style="width: 100%;" value="<?= $tbl_image ?>"></td>
</tr>
<tr>
...
while ($row = mysql_fetch_row($res)) {
	...
	$res2 = mysql_query("SELECT * FROM ".$PREFIX."catalog_models WHERE brand_id = $row[0] ORDER by position");
