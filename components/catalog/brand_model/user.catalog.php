...
if(strpos($id, "item-")===false && strpos($id, "model-")===false && strpos($id, "section-")===false && strpos($id, "price-")===false && strpos($id, "plastic")===false && strpos($id, "waitingfor")===false) {
    ...
    if($row=mysql_fetch_row($res)) {
        ...
        if($_SESSION['userid'] && $_SESSION['actype'][0]==1) {
            ...
            $ufl=1;
            $res2=mysql_query("SELECT b.id,b.name AS position_name,b.oem,c.name AS modelname,c.altname AS altmodelname,b.price,b.quantity,b.waitingfor,b.description,b.special,b.available,b.section,b.country,b.msk,b.spb FROM ".$PREFIX."catalog_items AS b, ".$PREFIX."catalog_models AS c WHERE b.model_id = c.id $arcquery AND (b.model_id = " . $idpos . " OR b.model_variants LIKE '%{$row[1]}%') ORDER BY b.description");
        }
        $s=0;
        while($row2=mysql_fetch_row($res2)){
            $s++;
            ...
            if($_SESSION['userid'] && $_SESSION['trend_purc'][$row2[12]."_".$row2[11]]!=='0' && $_SESSION['trend_purc'][$row2[12]."_".$row2[11]]!==null && $_SESSION['actype'][2]==1) {
                $row2[5]=floor($row2[5]*(100-$_SESSION['trend_purc'][$row2[12]."_".$row2[11]])/100);
            } else {
                $row2[5]=($_SESSION['userid'] && $_SESSION['percent']>0 && $_SESSION['actype'][0]!=1)?floor($row2[5]*(100-$_SESSION['percent'])/100):$row2[5];
            }
    ...
    if(array_key_exists($idpos, $sections)) {
        ...
        if($_SESSION['userid'] && $_SESSION['actype'][0]==1) {
            ...
            $res2=mysql_query("SELECT b.id,b.name AS position_name,b.oem,c.name AS modelname,c.altname AS altmodelname,b.price,b.quantity,b.waitingfor,b.description,b.special,b.available,d.name,b.country AS brandname,b.msk,b.spb FROM ".$PREFIX."catalog_items AS b, ".$PREFIX."catalog_models AS c, ".$PREFIX."catalog_brands AS d WHERE b.model_id = c.id AND b.brand_id = d.id $arcquery AND b.section = " . $idpos . " ORDER BY d.name,c.name,b.description");
        }
        $s=0;
        while($row2=mysql_fetch_row($res2)){
            $s++;
    ...
    if ($row = mysql_fetch_array($res)) {
        ...
        if($_SESSION['userid'] && $_SESSION['trend_purc'][$row['country']."_".$row['section']]!=='0' && $_SESSION['trend_purc'][$row['country']."_".$row['section']]!==null && $_SESSION['actype'][2]==1 && $row['special']=='') {
            $row['price']=floor($row['price']*(100-$_SESSION['trend_purc'][$row['country']."_".$row['section']])/100);
        } else {
        ...
        while($row2=mysql_fetch_array($res2)){
            ...
            if($_SESSION['actype'][0]==1) {
                 ...
                 $row2['price']=floor($row2['price']*(100-$_SESSION['trend_purc'][$row2['country']."_".$row2['section']])/100);
            } elseif($_SESSION['userid'] && $_SESSION['percent']>0 && $_SESSION['actype'][0]!=1) {
                 $row2['price']=floor($row2['price']*(100-$_SESSION['percent'])/100);
            }
    ...
    if($id=="plastic") {
        ...
        while($row2=mysql_fetch_row($res2)){
            ...
            if($_SESSION['userid'] && $_SESSION['trend_purc'][$row2[13]."_".$row2[12]]!=='0' && $_SESSION['trend_purc'][$row2[13]."_".$row2[12]]!==null && $_SESSION['actype'][2]==1 && $row2[9]=='') {
                $row2[5]=floor($row2[5]*(100-$_SESSION['trend_purc'][$row2[13]."_".$row2[12]])/100);
            } else {
                $row2[5]=($_SESSION['percent']>0 && $_SESSION['actype'][0]!=1 && $row2[9]=='')?floor($row2[5]*(100-$_SESSION['percent'])/100):$row2[5];
            }
            //скидка 
            if($row2[9]!='') {
                $row['special_price']=$price;
                ...
                if($_SESSION['userid'] && $_SESSION['trend_purc'][$row2[13]."_".$row2[12]]!=='0' && $_SESSION['trend_purc'][$row2[13]."_".$row2[12]]!==null && $_SESSION['actype'][2]==1) {
                    ...
                    $row2[5]=($_SESSION['percent']>0 && $_SESSION['actype'][0]!=1)?floor($spec*(100-$_SESSION['percent'])/100):$spec;
                }
            }
